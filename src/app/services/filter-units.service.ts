...
type HOURS_INDEXES = 'morning' | 'afternoon' | 'night'
...
export class FilterUnitsService {
  ...
  filter(results: Location[], showClosed: boolean, hour: string){
    ...
    if(hour){
      const OPEN_HOUR = OPENING_HOURS[hour as HOURS_INDEXES].first
      const CLOSE_HOUR = OPENING_HOURS[hour as HOURS_INDEXES].last
